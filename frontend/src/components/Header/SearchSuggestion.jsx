...
const useStyles = makeStyles(() => ({
  searchContainer: {
    ...
    boxShadow: '0 0.313rem 0.625rem rgba(0, 0, 0, 0.08)',
  ...
  search: {
    position: 'relative',
    flexDirection: 'row',
    boxShadow: '0rem 0.3125rem 0.625rem rgba(0, 0, 0, 0.08)',
...
function SearchSuggestion({ setSearchCurrentValue = () => {} }) {
  ...
  const handleSuggestionSelected = (event) => {
    ...
    if (name?.includes(':')) {
      ...
      navigate(`/image/${splitName[0]}/tag/${splitName[1]}`);
    } else {
      navigate(`/image/${name}`);
    }
  };
  ...
  const renderSuggestions = () => {
    return suggestionData.map((suggestion, index) => (
      <ListItem
        ...
        style={highlightedIndex === index ? { backgroundColor: '#f0f7ff' } : {}}
      ...
      >
        ...
        {isOpen && isLoading && !isEmpty(searchQuery) && isEmpty(suggestionData) && (
          <>
            <ListItem
              className={classes.searchItem}
              style={{ color: '#6b7280', fontSize: '1rem', textOverflow: 'ellipsis' }}
