...
func (service *BaseService) syncRef(ctx context.Context, localRepo string, remoteRepo string, remoteImageRef, localImageRef ref.Ref,
	...
	// Check if platform filters are configured
	platforms := service.contentManager.GetPlatforms(remoteRepo)
	if len(platforms) > 0 {
		platStrs := make([]string, 0, len(platforms))
		for _, p := range platforms {
			if p.OS != "" && p.Arch != "" {
				platStrs = append(platStrs, p.OS+"/"+p.Arch)
			}
		}
		if len(platStrs) > 0 {
			service.log.Info().Strs("platforms", platStrs).Str("repo", remoteRepo).
				Msg("filtering sync by platform")
			copyOpts = append(copyOpts, regclient.ImageWithPlatforms(platStrs))
		}
	}
