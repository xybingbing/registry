...
// GetPlatforms returns the platform filter for a repo, or nil if no filter is set.
func (cm ContentManager) GetPlatforms(repo string) []syncconf.Platform {
	content := cm.getContentByUpstreamRepo(repo)
	if content == nil || len(content.Platforms) == 0 {
		return nil
	}
