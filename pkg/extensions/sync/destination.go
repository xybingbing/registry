...
func (registry *DestinationRegistry) copyManifest(repo string, desc ispec.Descriptor,
	...
	*seen = append(*seen, desc.Digest)
	...
	case ispec.MediaTypeImageIndex, mediatype.Docker2ManifestList:
			...
			if err != nil {
				...
				if errors.Is(err, zerr.ErrBlobNotFound) || errors.As(err, &pathNotFoundErr) {
					registry.log.Warn().Err(err).Str("dir", path.Join(tempImageStore.RootDir(), repo)).
						...
						Msg("skipping missing manifest blob in image index, continuing sync with other manifests (filtered by platform?")
		...
		// When platform filtering is enabled, some sub-manifests referenced in the index
		// may not have been pulled. Remove those entries from the index before uploading.
		filteredManifests := make([]ispec.Descriptor, 0, len(indexManifest.Manifests))
		for _, m := range indexManifest.Manifests {
			_, err := tempImageStore.GetBlobContent(repo, m.Digest)
			if err != nil {
				registry.log.Warn().Err(err).Str("digest", m.Digest.String()).
					Msg("removing platform-specific manifest that was not synced from the index manifest")
				continue
			}
		...
		if err != nil {
			registry.log.Error().Str("errorType", common.TypeOf(err)).Err(err).
				Msg("failed to marshal filtered index manifest")
